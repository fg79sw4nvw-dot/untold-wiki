...
| ネメリア | `(5,200,3,800)` | 西部森林 |
| エルド | `(8,000,4,200)` | 中央草原北寄り |
| アウロス | `(10,700,3,700)` | 北東高原 |
| ヴェルノク | `(8,400,6,000)` | 中央草原南寄り |
| ネレム | `(6,000,6,100)` | 南西湿地 |
| モルグレイ | `(11,200,6,100)` | 東〜南東荒野 |
| マレスタ | 承認済みの東寄り配置 | 南岸・主河川河口の東側 |

ヴェルノクは旧基準 `(8,500,5,500)` から南へ500px、西へ100px移動した位置を新基準とする。

マレスタは最後に承認した東寄り位置を地理上の確定位置とし、実装用の最終数値座標のみ後で正規化する。

## 周辺有人島

- スカルド：北西有人島内の最後に承認した位置を地理上の確定位置とする。
- サヴェリア：東有人島。基準座標 `(14,600,5,800)`。
...
- 主島の地形割り当て
- 町の地理上の配置
- ヴェルノクの新基準座標 `(8,400,6,000)`
- 主河川が南西湖沼水系を経てマレスタ西側から外海へ流入する構造
- 探索済み範囲だけを記録するMAP方式

## 後から決めるもの

- 主島の正式名称
- マレスタとスカルドの実装用最終数値座標への正規化
- 無人島4島の名称・用途・内容
- 川幅、支流、橋、渡河地点の詳細
- 航路と船移動の所要時間
- MAPへ記録される探索半径・判定方式
