...
- **UNTOLDの会話では、製作者自身の考え方・価値観・制作観・AI観・判断基準等に意味のある変化や自己理解が生じたかを随時判断し、記録価値がある場合は `pages/journal/creator-reflection-log.md` へ原則自動で追記する。** 毎回ユーザーへ記録確認を求めず、通常の仕様決定や単純な作業進捗は成長記録へ混ぜない。詳細な記録基準は同ページの「自動記録ルール」を正とする。
